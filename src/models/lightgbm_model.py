"""LightGBM player performance prediction model.

Trains a LightGBM regressor to predict FPL points for the next gameweek.
Includes early stopping, model serialization, MLflow logging, and
a prediction pipeline for both training and inference.
"""
...
MODEL_FILE = MODEL_DIR / "player_predictor.joblib"
...
def train_player_model(
    ...
    logger.info(
        "Trained LightGBM model: %d features, best_iteration=%d, time=%.1fs",
        info["n_features"],
        info["best_iteration"],
...
def predict_points(
    ...
    Returns:
        Array of predicted points (clipped to >= 0).
    """
    raw_predictions = model.predict(x)
    # FPL points can't be negative
    predictions: npt.NDArray[np.float64] = np.clip(
        np.asarray(raw_predictions), 0, None
    ).astype(np.float64)
    return predictions
...
def _log_training_to_mlflow(
    ...
    try:
        ...
        with mlflow.start_run(run_name="player_predictor"):
            # Log parameters
