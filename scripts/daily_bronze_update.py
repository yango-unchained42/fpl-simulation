...
def get_source_hash(table_name: str, season: str = CURRENT_SEASON) -> str:
...
def has_source_changed(table_name: str, season: str = CURRENT_SEASON) -> bool:
...
def fetch_fpl_players() -> pl.DataFrame:
    ...
    season_dir = DATA_DIR / CURRENT_SEASON
...
def update_fpl_bronze() -> bool:
    ...
    if has_source_changed("fpl_players"):
        ...
        if not df.is_empty():
            ...
            update_metadata("fpl_players", CURRENT_SEASON, len(df))
            updated = True
    else:
    ...
    if has_source_changed("fpl_teams"):
        ...
        if not df.is_empty():
            ...
            update_metadata("fpl_teams", CURRENT_SEASON, len(df))
            updated = True
    else:
    ...
    if has_source_changed("fpl_fixtures"):
        ...
        if not df.is_empty():
            ...
            update_metadata("fpl_fixtures", CURRENT_SEASON, len(df))
            updated = True
    else:
    ...
    if has_source_changed("fpl_gw"):
        ...
        if not df.is_empty():
            ...
            update_metadata("fpl_gw", CURRENT_SEASON, len(df))
            updated = True
    else:
...
def update_understat_bronze() -> bool:
    ...
    try:
        ...
        current_season_list = [CURRENT_SEASON.replace("-", "_")]
        ...
        if not df.is_empty():
            ...
            update_metadata("understat_player_match_stats", CURRENT_SEASON, len(df))
            updated = True
        ...
        if not df.is_empty():
            ...
            update_metadata("understat_shots", CURRENT_SEASON, len(df))
            updated = True
        ...
        if not df.is_empty():
            ...
            update_metadata("understat_match_stats", CURRENT_SEASON, len(df))
            updated = True
